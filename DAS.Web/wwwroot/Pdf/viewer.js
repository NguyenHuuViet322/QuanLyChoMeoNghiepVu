    ...
    this.draw = function pageviewDraw(callback) {
        ...
        function pageViewDrawCallback(error) {
            ...
            if (self.loadingIconDiv) {
                ...
                if (jQuery(document).find('.pdfViewer').find('.page[data-rotate-inited="true"]').length != 0) {
                    rotate_Page_pdf(0, "allPage");
                }
...
var PDFViewerApplication = {
    ...
    get pagesCount() {
        if (!this.pdfDocument || !Object.keys(this.pdfDocument).includes(numPages)) {
            return 0;
        }
    ...
    get page() {
        //if (!this.pdfViewer) {
        //    return 0; 
        //} //Dùng chỗ này Summernote bị lỗi (ko nhập k,p,n... được) (ko return)
        return this.pdfViewer.currentPageNumber;
    },
    ...
    download: function pdfViewDownload() {
        ...
        //Disable download
        //if (Cdata.Storage.isDisableDownfile) {
        //    return false;
        //}
...
function webViewerInitialized() {
    ...
    //var file = 'http://local.docpro.vn/storage/Files/fsi.pdf';
...
jQuery(document).ready(function () {
    ...
    //Disabled download
    //if (Cdata.Storage.isDisablePrintfile) {
    //    jQuery("body").append("<style>@media print {html, body {display: none;}}</style><style type='text/css' media='print'>* { display: none; }</style>");
    ...
    //            return false;
    //        });
    //        jQuery(window).on('afterprint', function () {
    //            return false;
    //        });
    //    }
...
function rotate_Page_pdf(deg, type_rotate, rotate_page_number) {
    //deg: default = 0, rotate left = -90, rotate right = 90
    //type_rotate: "curentPage", "allPage", "targetPage"
    //rotate_page_number: if targetPage rotate_page_number = page number need rotate

    if (!deg) {
        deg = 0;
    }

    //CURENT PAGE OR ALL PAGE
    var target;
    if (type_rotate == "allPage") {
        target = jQuery(document).find('.pdfViewer').find('.page[data-rotate-inited="true"]');
    } else if (type_rotate == "curentPage") {
        var page_Number = jQuery(document).find("#pageNumber").val();
        target = jQuery(document).find("#pageContainer" + page_Number);
    } else {
        target = jQuery(document).find("#pageContainer" + rotate_page_number);
    }


    if (type_rotate == "allPage") {
        jQuery(document).find('.pdfViewer').find('.page[data-rotate-inited="true"]').each(function () {
            jQuery(this).find(".canvasWrapper").css("transform", "rotate(" + 0 + "deg)");
            jQuery(this).find(".textLayer").css("transform", "rotate(" + 0 + "deg)");

            var base_page_width = parseFloat($(this).css("width").replace("px", ""));
            var base_page_height = parseFloat($(this).css("height").replace("px", ""));

            var orientation = $(this).attr("data-orientation");
            var orientation_height, orientation_width;

            if (orientation == "vertical") {
                orientation_width = base_page_width > base_page_height ? base_page_height : base_page_width;
                orientation_height = base_page_width > base_page_height ? base_page_width : base_page_height;
            } else {
                orientation_width = base_page_width > base_page_height ? base_page_width : base_page_height;
                orientation_height = base_page_width > base_page_height ? base_page_height : base_page_width;
            }

            $(this).attr("data-inited-width", orientation_width);
            $(this).attr("data-inited-height", orientation_height);

            jQuery(this).find(".canvasWrapper").css("top", 0);
            jQuery(this).find(".canvasWrapper").css("right", 0);
            jQuery(this).find(".textLayer").css("top", 0);
            jQuery(this).find(".textLayer").css("right", 0);
    ...
    target.each(function () {
        ...
        if (!$(this).attr("data-inited-width") && !$(this).attr("data-inited-height")) {
            var page_width = $(this).css("width");
            var page_height = $(this).css("height");
            $(this).attr("data-inited-width", page_width);
            $(this).attr("data-inited-height", page_height);
        }

        var page_init_width = $(this).attr("data-inited-width");
        var page_init_height = $(this).attr("data-inited-height");

        var page_width_float = parseFloat(page_init_width.replace("px", ""));
        var page_height_float = parseFloat(page_init_height.replace("px", ""));
        var change_px = Math.abs(page_width_float - page_height_float) / 2;

        if (!$(this).attr("data-orientation")) {
            $(this).attr("data-orientation", page_width_float > page_height_float ? "horizontal" : "vertical")

        }
        //GET REG
        var curent_rotate;
        if ($(this).attr("data-rotate")) {
            curent_rotate = parseInt($(this).attr("data-rotate"));
        } else {
            curent_rotate = 0;
        }
        curent_rotate = (curent_rotate + 360 + deg) % 360;
        $(this).attr("data-rotate", curent_rotate);
        $(this).attr("data-rotate-inited", "true");
        $(this).find(".canvasWrapper").css({
            '-webkit-transform': 'rotate(' + curent_rotate + 'deg)',
            '-moz-transform': 'rotate(' + curent_rotate + 'deg)',
            '-ms-transform': 'rotate(' + curent_rotate + 'deg)',
            'transform': 'rotate(' + curent_rotate + 'deg)'
        });

        $(this).find(".canvasWrapper").css("transform-origin", "center");
        $(this).find(".textLayer").css({
            '-webkit-transform': 'rotate(' + curent_rotate + 'deg)',
            '-moz-transform': 'rotate(' + curent_rotate + 'deg)',
            '-ms-transform': 'rotate(' + curent_rotate + 'deg)',
            'transform': 'rotate(' + curent_rotate + 'deg)'
        ...
        $(this).find(".textLayer").css("transform-origin", "center");

        //FIX WITH HEIGHT TOP LEFT
        $(this).find(".canvasWrapper").css("position", "absolute");
        $(this).find(".canvasWrapper").css("bottom", "auto");
        $(this).find(".canvasWrapper").css("left", "auto");
        $(this).find(".textLayer").css("position", "absolute");
        $(this).find(".textLayer").css("bottom", "auto");
        $(this).find(".textLayer").css("left", "auto");
        if (curent_rotate == 90 || curent_rotate == 270) {
            $(this).css("width", page_init_height);
            $(this).css("height", page_init_width);

            $(this).find(".canvasWrapper").css("top", -change_px);
            $(this).find(".canvasWrapper").css("right", change_px);
            $(this).find(".textLayer").css("top", -change_px);
            $(this).find(".textLayer").css("right", change_px);
        } else {
            $(this).css("width", page_init_width);
            $(this).css("height", page_init_height);

            $(this).find(".canvasWrapper").css("top", 0);
            $(this).find(".canvasWrapper").css("right", 0);
            $(this).find(".textLayer").css("top", 0);
            $(this).find(".textLayer").css("right", 0);
        }
        $(this).find(".canvasWrapper").css("width", page_init_width);
        $(this).find(".canvasWrapper").css("height", page_init_height);
        $(this).find(".canvasWrapper").find('canvas').css("width", page_init_width);
        $(this).find(".canvasWrapper").find('canvas').css("height", page_init_height);
        $(this).find(".textLayer").css("width", page_init_width);
        $(this).find(".textLayer").css("height", page_init_height);
...
function set_page_rotate() {
    if ($("#RotateData").length != 0) {
        ...
        jQuery(document).find('.pdfViewer').find('.page[data-rotate-inited="true"]').each(function () {
            ...
            var rotate_class;
            if (rotate == 0) {
                rotate_class = "note_change_rotate";
            } else {
                rotate_class = "is_change_rotate";
            }
            var page_number = id.replace("pageContainer", "");
            var html = '<div data-page-number="' + page_number + '" class="form-group has-feedback rotate_form_group ' + rotate_class + '">' +
                '<label class="col-lg-4 control-label">Trang ' + page_number + '</label>' +
                '<div class="col-lg-8">' +
                '<input name="dataRotate_' + id + '" type="text" placeholder="Trang ' + page_number + '" class="pg_Rotate_input form-control noresize" value="' + rotate + '">' +
                '<a href="#" class="del_rotate_row" title="Xóa"><i class="ion-android-close"></i></a>' +
                '</div>' +
                '</div>';
            $("#RotateData").append(html);
        });
    }
}
function get_page_rotate() {

    if (jQuery(document).find("#RotateData").find(".rotate_form_group").length != 0) {
        var json_rotate = [];
        var obj = jQuery(document).find("#RotateData").find(".rotate_form_group");
        obj.each(function () {
            var page_number = $(this).attr("data-page-number");
            var deg = $(this).find(".pg_Rotate_input").val();
            var temp = new Object();
            temp["page_number"] = page_number;
            temp["deg"] = deg;
            json_rotate.push(temp);
        });
        for (var i = 0; i < json_rotate.length; i++) {
            rotate_Page_pdf(json_rotate[i].deg, "targetPage", json_rotate[i].page_number);
        }
...
jQuery(document).on('click', '#pageRotateCw', function () {
    rotate_Page_pdf(90, "curentPage");
});

jQuery(document).on('click', '#pageRotateCcw', function () {
    rotate_Page_pdf(-90, "curentPage");
});
