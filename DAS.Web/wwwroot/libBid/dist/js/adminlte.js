...
(function (global, factory) {
  ...
  var Default$a = {
    ...
    scrollbarTheme: $__default['default']('.control-sidebar-light').length > 0 ? 'os-theme-dark' : 'os-theme-light',
    scrollbarAutoHide: 'l',
  ...
  $__default['default']("" + SELECTOR_DROPDOWN_MENU).on('click', function (event) {
    if ($__default['default'](this).hasClass('dropdown-form')) {
      (function () {
        var events = $__default['default']._data(document, 'events') || {};
        events = events.click || [];

        var _loop = function _loop(i) {
          if (events[i].selector) {
            //Check if the clicked element matches the event selector
            if ($__default['default'](event.target).is(events[i].selector)) {
              events[i].handler.call(event.target, event);
            } // Check if any of the clicked element parents matches the delegated event selector (Emulating propagation)


            $__default['default'](event.target).parents(events[i].selector).each(function () {
  ...
  $__default['default'](SELECTOR_TABLE).on('ready', function () {
  ...
  var Default$6 = {
    scrollbarTheme: $__default['default']('[class*="sidebar-light-"]').length > 0 ? 'os-theme-dark' : 'os-theme-light',
    scrollbarAutoHide: 'l',
  ...
  var Layout = /*#__PURE__*/function () {
    ...
    _proto.fixLayoutHeight = function fixLayoutHeight(extra) {
      ...
      var $contentBodySelector = $contentSelector.find('.content');
      var $contentHeaderSelector = $contentSelector.find('.content-header');

      if (offset !== false) {
        if (max === heights.controlSidebar) {
          $contentSelector.css(this._config.panelAutoHeightMode, max + offset);
          $contentBodySelector.css('height', max + offset - $contentHeaderSelector.outerHeight());
        } else if (max === heights.window) {
          $contentSelector.css(this._config.panelAutoHeightMode, max + offset - heights.header - heights.footer);
          $contentBodySelector.css('height', max + offset - $contentHeaderSelector.outerHeight() - heights.header);
        } else {
        ...
        if (this._isFooterFixed()) {
          ...
          $contentBodySelector.css('height', parseFloat($contentSelector.css(this._config.panelAutoHeightMode)) + heights.footer);
        }
      }
